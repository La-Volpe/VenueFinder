...
class FavoriteRepositoryImpl(
    ...
    override suspend fun isFavorite(id: String): Boolean {
         return !getAllFavorite().firstOrNull { it.contains(id) }.isNullOrEmpty()
    }

    override suspend fun getAllFavorite(): Flow<Set<String>> {
        return dao.getAllFavorites().map { it.map { entity -> entity.id }.toSet() }
    }
}