...
class VenuesListContextTest {
	...
	@Test
	fun `locationFlow should return flow from observeLocationUpdates use case`() =
		runTest {
			// Given
			val expectedLocation = Location(60.169418, 24.931618, "Kamppi Chapel of Silence")
			every { observeLocationUpdates() } returns flowOf(expectedLocation)
	...
	@Test
	fun `getVenues should delegate to getNearbyVenues use case`() =
		runTest {
			// Given
			val lat = 60.169418
			val lon = 24.931618
	...
	@Test
	fun `favoritesFlow should delegate to getFavoritedVenuesUseCase`() =
		runTest {
			// Given
	...
	@Test
	fun `toggleFavorite should delegate to toggleFavoriteUseCase`() =
		runTest {
			// Given
	...
	@Test
	fun `getVenues should handle invalid latitude and longitude inputs`() =
		runTest {
			// Given
	...
	@Test
	fun `favoritesFlow should handle empty or null data`() =
		runTest {
			// Given
