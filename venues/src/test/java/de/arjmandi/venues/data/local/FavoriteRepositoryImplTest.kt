...
class FavoriteRepositoryImplTest {
    ...
    @Test
    fun `toggleFavorite should insert if not exists`() = runTest {
    ...
    @Test
    fun `toggleFavorite should delete if exists`() = runTest {
    ...
    @Test
    fun `isFavorite should return true when venue is favorited`() = runTest {
        coEvery { dao.getAllFavorites() } returns flowOf(listOf(FavoriteEntity("venue-1")))
    ...
    @Test
    fun `isFavorite should return false when venue is not favorited`() = runTest {
        coEvery { dao.getAllFavorites() } returns flowOf(emptyList())
        val isFavorite = repository.isFavorite("venue-1")
